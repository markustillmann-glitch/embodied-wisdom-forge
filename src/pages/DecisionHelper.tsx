...
import { ArrowLeft, Send, Loader2, RotateCcw, Scale, Save, X, Sparkles } from "lucide-react";
...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/decision-helper-chat`;
...
const DecisionHelper = () => {
  ...
  const startSession = async () => {
    ...
    try {
      await streamChat([{ role: "user", content: "Ich brauche Hilfe bei einer Entscheidung." }]);
    } catch (err) {
  ...
  const saveToVault = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('memories').insert({
        ...
        memory_type: 'decision-helper',
        emotion: 'reflection',
        summary: `Entscheidungshilfe vom ${new Date().toLocaleDateString('de-DE')}`
      });
    ...
    <div className="min-h-[100dvh] bg-background flex flex-col overflow-hidden">
      ...
      <section className="pt-14 pb-2 sm:pt-24 sm:pb-8 relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col items-center text-center">
            ...
            >
              Bedürfnisorientierte Entscheidungsfindung mit dem Oria-Modell
            </motion.p>
          </div>
        </div>
      </section>
        ...
        {!hasStarted ? (
          ...
          >
            ...
            <h2 className="text-lg sm:text-xl font-serif text-foreground mb-2 sm:mb-3">
              Welche Entscheidung beschäftigt dich?
            </h2>
            <p className="text-sm text-muted-foreground max-w-md mb-4 sm:mb-6 leading-relaxed">
              Ich begleite dich durch eine bedürfnisorientierte Entscheidungsfindung. 
              Wir schauen gemeinsam, welche Bedürfnisse bei deiner Entscheidung eine Rolle spielen 
              und wie du zu einer stimmigen Wahl kommst.
            </p>
            <p className="text-xs text-muted-foreground/70 max-w-sm mb-6 sm:mb-8 italic">
              Ca. 5-10 Minuten – danach kannst du in Oria vertiefen.
            </p>
            <Button
      ...
      <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
        <DialogContent className="sm:max-w-md mx-4 rounded-xl">
          <DialogHeader>
            <DialogTitle className="text-lg">Entscheidung speichern?</DialogTitle>
            <DialogDescription className="text-sm">
              Speichere diese Reflexion für später im Erinnerungstresor.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="z.B. Jobwechsel-Entscheidung"
