...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/life-checkin-chat`;
...
const LifeCheckin = () => {
  ...
  const startSession = async () => {
    ...
    try {
      await streamChat([{ role: "user", content: "Hallo, ich möchte meinen Life Check-in machen." }]);
    } catch (error) {
  ...
  const saveToVault = async () => {
    ...
    try {
      ...
      // Extract life area and need from conversation
      const lastAssistantMsg = [...messages].reverse().find(m => m.role === 'assistant');
      let summary = `Life Check-in vom ${format(new Date(), 'dd.MM.yyyy', { locale: de })}`;
      
      if (lastAssistantMsg?.content) {
        const areaMatch = lastAssistantMsg.content.match(/Lebensbereich:\s*([^\n]+)/);
        const needMatch = lastAssistantMsg.content.match(/Bedürfnis:\s*([^\n]+)/);
        if (areaMatch || needMatch) {
          summary = [
            areaMatch ? areaMatch[1].trim() : null,
            needMatch ? `→ ${needMatch[1].trim()}` : null
          ].filter(Boolean).join(' ');
        }
      }
    ...
    <div className="min-h-[100dvh] bg-background flex flex-col overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col items-center text-center">
            <motion.div
              ...
              className="mb-1 sm:mb-4 flex items-center gap-4"
            >
              <Link
                to="/oria-apps"
              ...
              <Link
                to="/life-checkin-modell"
                className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-accent transition-colors touch-manipulation"
              >
            ...
            >
              Bedürfnis- und erinnerungsbasierter Selbstkontakt in 10 Lebensbereichen
            </motion.p>
          </div>
        </div>
      </section>
        ...
        {!hasStarted ? (
          ...
          >
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-accent/10 flex items-center justify-center mb-4 sm:mb-6">
              <Compass className="w-7 h-7 sm:w-8 sm:h-8 text-accent" />
            </div>
            <h2 className="text-lg sm:text-xl font-serif text-foreground mb-2 sm:mb-3">
              Wie spricht dein Leben heute mit dir?
            </h2>
            <p className="text-sm text-muted-foreground max-w-md mb-4 sm:mb-6 leading-relaxed">
              In 5–7 Minuten begleite ich dich durch einen sanften Check-in in 10 Lebensbereichen. 
              Nicht messen, sondern spiegeln – für echten Selbstkontakt.
            </p>
            <p className="text-xs text-muted-foreground/70 max-w-sm mb-6 sm:mb-8 italic">
              Deine Check-ins kannst du speichern und monatlich reflektieren.
            </p>
            <Button
            ...
            {user && pastCheckins.length > 0 && (
              ...
              >
                ...
                >
                  ...
                  <span>Vergangene Life Check-ins ({pastCheckins.length})</span>
              ...
              <div className="flex gap-2 items-end">
                ...
                <div className="flex-1 relative">
                  <Textarea
                    ...
                    placeholder="Teile, was dich bewegt..."
                    className="min-h-[48px] sm:min-h-[52px] max-h-32 resize-none pr-12 text-base"
                    disabled={isLoading}
                  />
                  <Button
                    onClick={sendMessage}
                    ...
                    size="icon"
                    className="absolute right-1.5 bottom-1.5 h-9 w-9 bg-accent text-accent-foreground hover:bg-accent/90 touch-manipulation active:scale-95"
                  >
      ...
      <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
        <DialogContent className="sm:max-w-md mx-4 rounded-xl">
          <DialogHeader>
            <DialogTitle className="font-serif">Check-in speichern?</DialogTitle>
            <DialogDescription>
              Speichere diesen Life Check-in in deinem Tresor für die monatliche Reflexion.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <Input
              ...
              placeholder="z.B. Montag-Morgen Reflexion"
              className="text-base"
            />
          </div>
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button
              ...
              className="bg-accent text-accent-foreground hover:bg-accent/90 order-1 sm:order-2"
            >
              {isSaving ? (
