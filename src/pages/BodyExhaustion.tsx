...
import { ArrowLeft, Send, Loader2, RotateCcw, Activity, Save, X, Sparkles } from "lucide-react";
...
const CHAT_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/body-exhaustion-chat`;

const BodyExhaustion = () => {
  const { user } = useAuth();
  ...
  const startSession = async () => {
    ...
    try {
      await streamChat([{ role: "user", content: "Ich möchte meinen körperlichen Erschöpfungszustand erfassen." }]);
    } catch (err) {
  ...
  const saveToVault = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('memories').insert({
        ...
        memory_type: 'body-exhaustion',
        emotion: 'body-awareness',
        summary: `Körper-Check vom ${new Date().toLocaleDateString('de-DE')}`
      });
  ...
  const deepenInOria = () => {
    ...
    sessionStorage.setItem('coach-deepen-context', JSON.stringify({
      context,
      topic: 'Körperliche Erschöpfung',
      source: 'body-exhaustion'
    }));
    ...
    <div className="min-h-[100dvh] bg-background flex flex-col overflow-hidden">
      ...
      <section className="pt-14 pb-2 sm:pt-24 sm:pb-8 relative overflow-hidden shrink-0">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col items-center text-center">
            ...
            >
              Körperlichen Erschöpfungszustand erfassen – sanft und achtsam
            </motion.p>
          </div>
        </div>
      </section>
      ...
      <section className="flex-1 flex flex-col max-w-3xl mx-auto w-full px-3 sm:px-6 pb-2 sm:pb-6 min-h-0 overflow-hidden">
        {!hasStarted ? (
          <motion.div
            ...
            className="flex-1 flex flex-col items-center justify-center text-center py-4 sm:py-12 px-2"
          >
            <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-accent/10 flex items-center justify-center mb-4 sm:mb-6">
              <Activity className="w-7 h-7 sm:w-8 sm:h-8 text-accent" />
            </div>
            ...
            <p className="text-sm text-muted-foreground max-w-md mb-4 sm:mb-6 leading-relaxed">
              In 5-10 Minuten erfasse ich mit dir deinen aktuellen körperlichen Zustand. 
              Wir schauen gemeinsam auf Energie, Anspannung, Schlaf und Signale deines Nervensystems.
            </p>
            <p className="text-xs text-muted-foreground/70 max-w-sm mb-6 sm:mb-8 italic">
              Keine Diagnose – nur achtsame Selbstwahrnehmung. Danach kannst du in Oria vertiefen.
            </p>
            <Button
            ...
            <div className="shrink-0 border-t border-border pt-3 sm:pt-4 pb-3 sm:pb-0 bg-background">
              <div className="flex gap-2 items-end">
                <Button
                  ...
                  className="shrink-0 h-11 w-11 sm:h-10 sm:w-10 touch-manipulation"
                >
                ...
                <div className="flex-1 relative">
                  <Textarea
                    ...
                    disabled={isLoading}
                    className="min-h-[48px] sm:min-h-[52px] max-h-32 resize-none pr-12 text-base sm:text-sm"
                    rows={1}
                  />
                  <Button
                    onClick={sendMessage}
                    ...
                    size="icon"
                    className="absolute right-1.5 bottom-1.5 h-9 w-9 sm:h-8 sm:w-8 bg-accent text-accent-foreground hover:bg-accent/90 touch-manipulation"
                  >
      ...
      <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
        <DialogContent className="sm:max-w-md mx-4 rounded-xl">
          <DialogHeader>
            <DialogTitle className="text-lg">Körper-Check speichern?</DialogTitle>
            <DialogDescription className="text-sm">
              Speichere diese Erfassung für später im Erinnerungstresor.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="z.B. Erschöpfung nach Projekt"
