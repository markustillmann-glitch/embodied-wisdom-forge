...
const Resonanzradar = () => {
  ...
  return (
    <div className="min-h-[100dvh] bg-background flex flex-col overflow-hidden">
      ...
      {/* Hero Section - Sehr kompakt auf Mobile */}
          ...
          <div className="flex flex-col items-center text-center">
            ...
            >
              ...
              >
                ...
                <span className="hidden sm:inline">{t('resonanzradar.backToApps')}</span>
      ...
      {/* Chat Area - Optimiert für Mobile */}
      <section className="flex-1 flex flex-col max-w-3xl mx-auto w-full px-3 sm:px-6 pb-2 sm:pb-6 min-h-0 overflow-hidden">
        {!hasStarted ? (
          ...
          >
            ...
            <p className="text-sm sm:text-sm text-muted-foreground max-w-md mb-4 sm:mb-6 leading-relaxed">
              {t('resonanzradar.welcomeDesc')}
            </p>
            <p className="text-xs sm:text-xs text-muted-foreground/70 max-w-sm mb-6 sm:mb-8 italic">
          ...
          <>
            {/* Messages - Optimiertes Scrolling */}
            <div className="flex-1 overflow-y-auto py-3 sm:py-4 space-y-3 sm:space-y-4 min-h-0 overscroll-contain">
              {messages.map((msg, idx) => (
                ...
                >
                  <div
                    className={`max-w-[88%] sm:max-w-[85%] rounded-2xl px-4 py-3 sm:px-4 sm:py-3 ${
                      msg.role === "user"
                        ? "bg-accent text-accent-foreground rounded-br-md"
                        : "bg-card border border-border rounded-bl-md shadow-sm"
                    }`}
                  >
                    <p className="text-sm sm:text-sm whitespace-pre-wrap leading-relaxed">
                      {msg.content}
                    </p>
                  </div>
                </motion.div>
      ...
      {/* Save to Vault Dialog - Mobile-optimiert */}
      <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
        <DialogContent className="sm:max-w-md mx-4 rounded-xl">
          <DialogHeader>
            <DialogTitle className="text-lg">{t('resonanzradar.saveDialogTitle')}</DialogTitle>
